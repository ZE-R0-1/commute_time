...
class BusArrivalService {
  ...
  static Future<BusArrivalInfo?> getBusArrivalItemv2(String stationId, String routeId, int staOrder) async {
    try {
      final apiKey = dotenv.env['GYEONGGI_BUS_API_KEY'] ?? '';
      if (apiKey.isEmpty) {
        print('❌ 경기도 버스 API 키가 없습니다.');
      ...
      if (response.statusCode == 200) {
        return _parseArrivalItemResponse(response.body);
      } else {
        print('❌ API 호출 실패: ${response.statusCode}');
        return null;
      }
  ...
  static BusArrivalInfo? _parseArrivalItemResponse(String jsonString) {
    try {
      final data = jsonDecode(jsonString);
      final response = data['response'];
      final msgBody = response['msgBody'];
      ...
      if (busArrivalItem != null) {
        ...
        final arrivalInfo = BusArrivalInfo(
          routeId: busArrivalItem['routeId']?.toString() ?? '',
          routeName: busArrivalItem['routeName']?.toString() ?? '',
          routeTypeName: routeTypeName,
          stationId: busArrivalItem['stationId']?.toString() ?? '',
          stationName: '', // v2 API에서는 stationName이 없음
          predictTime1: int.tryParse(busArrivalItem['predictTime1']?.toString() ?? '0') ?? 0,
          predictTime2: int.tryParse(busArrivalItem['predictTime2']?.toString() ?? '0') ?? 0,
          locationNo1: int.tryParse(busArrivalItem['locationNo1']?.toString() ?? '0') ?? 0,
          locationNo2: int.tryParse(busArrivalItem['locationNo2']?.toString() ?? '0') ?? 0,
          lowPlate1: busArrivalItem['lowPlate1']?.toString() == '1' ? 'Y' : 'N',
          lowPlate2: busArrivalItem['lowPlate2']?.toString() == '1' ? 'Y' : 'N',
          plateNo1: busArrivalItem['plateNo1']?.toString() ?? '',
          plateNo2: busArrivalItem['plateNo2']?.toString() ?? '',
          remainSeatCnt1: int.tryParse(busArrivalItem['remainSeatCnt1']?.toString() ?? '0') ?? 0,
          remainSeatCnt2: int.tryParse(busArrivalItem['remainSeatCnt2']?.toString() ?? '0') ?? 0,
          staOrder: int.tryParse(busArrivalItem['staOrder']?.toString() ?? '0') ?? 0,
        );
